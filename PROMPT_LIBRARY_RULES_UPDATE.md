...
- Library name: AG Charts Angular (JWebMP plugin)
- Current/new version: <VERSION>
- Repository URL / path: https://github.com/JWebMP/AgCharts.git
- Rules Repository Location: rules/generative/frontend/jwebmp/agcharts
- Short description: A plugin for the JWebMP renderer that leverages the TypeScript and Angular plugins to render Ag Charts.
- Type:
...
- MCP servers to register (Mermaid MCP required; add others as needed): list name/purpose/endpoint/type (Mermaid MCP `https://mcp.mermaidchart.com/mcp` type `http`). Keep secrets out of the repo; reference env var names instead.
  - Mermaid MCP — http `https://mcp.mermaidchart.com/mcp` (type `http`)
  - Ag Grid MCP — stdio `ag-mcp` via `npx`:
    ```json
    "ag-mcp": {
        "type": "stdio",
        "command": "npx",
...
- Component/topic areas (list): Ag Charts Angular (JWebMP wrapper), chart components, Angular 20 integration, JWebMP client/TypeScript bridge
...
- For this run, blanket approval is selected; treat all STOP gates as auto-approved while recording that policy in outputs.
